        ...
        public string CheckAreaParameters(List<string> plotNames, FilteredElementCollector Areas)
        {
            ...
            foreach (Area area in Areas)
            {
                if (area.Area != 0)
                {
                    ...
                    if (!hasValue(area.LookupParameter("A Instance Area Category"))) { errorMessage += $"Грешка: Area {area.LookupParameter("Number").AsString()} / id: {area.Id.ToString()} / Непопълнен параметър: A Instance Area Category\n"; }
